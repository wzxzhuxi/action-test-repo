...
void test_greet() {
    assert(action_test_repo::greet("World") == "Hello, World!");
    assert(action_test_repo::greet("") == "Hello, !");
    std::cout << "[PASS] test_greet" << std::endl;
}

void test_add() {
    assert(action_test_repo::add(2, 3) == 5);
    assert(action_test_repo::add(-1, 1) == 0);
    assert(action_test_repo::add(0, 0) == 0);
    std::cout << "[PASS] test_add" << std::endl;
}

void test_parse_int_success() {
    auto result = action_test_repo::parse_int("42");
    assert(result.is_ok());
    assert(result.value() == 42);

    auto negative = action_test_repo::parse_int("-123");
...
void test_parse_int_failure() {
    auto result = action_test_repo::parse_int("abc");
    assert(result.is_err());

    auto empty = action_test_repo::parse_int("");
    assert(empty.is_err());

    auto trailing = action_test_repo::parse_int("42abc");
...
void test_result_map() {
    auto result = action_test_repo::parse_int("10");
    auto doubled = result.map([](int x) { return x * 2; });
    assert(doubled.is_ok());
    assert(doubled.value() == 20);

    auto err = action_test_repo::parse_int("invalid");
